...
Only keys defined by the shared launch contract are accepted once UI testing is enabled. Functional tests use the `functional` profile, screenshot staging uses `marketing`, and oversized/scroll coverage opts into `stress`. Direct routes must refer to a post in the selected profile. A story route cannot use the in-app browser, and fixture article mode fails closed if the selected post has no local article. Commenting is off in release-build runtime policies; DEBUG builds enable it by default, with a `disableCommenting` launch argument to opt out locally (non-persisting, absent from release builds, ignored when typed UI testing is active).
